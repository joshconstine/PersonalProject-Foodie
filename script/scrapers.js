...
async function scrapeRestaurant(city, state) {
  const url = `https://www.yelp.com/search?find_desc=restaurants&find_loc=${city}%2C+${state}`;
  ...
  for (let i = 9; i < 14; i++) {
    const [el] = await page.$x(
      `//*[@id="main-content"]/div/ul/li[${i}]/div/div/div/div[2]/div[1]/div[1]/div[1]/div/div/h3/span/a`
    );
    const nameSrc = await el.getProperty("textContent");
    const nameSrcText = await nameSrc.jsonValue();

    const [el2] = await page.$x(
      `//*[@id="main-content"]/div/ul/li[${i}]/div/div/div/div[1]/div/div/div/div/div/div[1]/div/a/img`
    );
    const imageSrc = await el2.getProperty("src");
    const imageSrcText = await imageSrc.jsonValue();

    const [el3] = await page.$x(
      `//*[@id="main-content"]/div/ul/li[${i}]/div/div/div/div[2]/div[1]/div[1]/div[3]/div/div/div/p/span[1]/a[1]/button/p`
    );
